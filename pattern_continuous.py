# -*- coding: utf-8 -*-
"""Pattern_Continuous.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1kanfNzO2_Y3RUPJrZhPgUR1Ui5NeOWR0
"""
...
for ksize in [15,21,31,63]:
    for theta in np.arange(0, np.pi, np.pi / 6):
        kern = cv2.getGaborKernel((ksize, ksize), 4., theta, 12.0, 0.5, 0, ktype=cv2.CV_32F)  # Constructing the Gabor Kernels at different orientations 
...
def process(img, filters):
    accum = []
    for kern in filters:
        fimg = cv2.filter2D(img, cv2.CV_8UC3, kern)   #Filtering the image using the Gabor Kernels
        accum.append(fimg)
    return accum

FF = np.zeros((1,1),dtype = np.double)

class drlse_pattern(object):  #Implemted the distance regularised level set
...
class levelSet_pattern(object):
    ...
    def initializePhiAtScribble(self,image,x,y):  #Initialising phi for propagation
        c0 = 3
    ...
    def pattern2shading(self, image, rgb):   #Implementing the pattern2shading using the equaitions given in the paper
    ...
    def strokepreserving(self, image, rgb):    #Implementing the pattern2shading using the equaitions given in the paper
        img = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        color = np.array(rgb)
        colored_img = np.ones((self.phi.shape[0],self.phi.shape[1],3), dtype=np.uint8)
        colored_img[self.phi<0] = color
    ...
    def compute_feats(self, filtered):  #Computing mean and varience to compute feature vector as shown in the paper
    ...
    def calculateF_pattern(self,image,x,y):   # In this function we are calculating the coefficient H in the equation
        w = 8                                 # H calculates the difference between feature vectors of the skribble and the window
        ...
        if FF.shape[0] == 1:
            ...
            for i in range(w, padded[0].shape[0]-w, step):
                for j in range(w, padded[0].shape[1]-w, step):
                    ...
                    F[i-w:i-w+step, j-w:j-w+step] = np.sum((feats - scribble_feats)**2) #Calculating difference in features between the user scribble and the rest of the image
            FF = F
            print("done!")
        else:
            F = copy.deepcopy(FF)
    ...
    def gradientDescent(self,image,x,y):   #Implemented Gradient Descent impemented in paper
